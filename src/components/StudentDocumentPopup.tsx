...
export function StudentDocumentPopup() {
  ...
  const dismissDocument = async (docId: string) => {
          ...
          {visible.map((doc) => (
            <div key={doc.id} className="bg-white rounded-lg shadow-lg border border-blue-100 p-4 animate-slide-up">
              <div className="flex items-start justify-between mb-2">
                ...
                <button onClick={() => dismissDocument(doc.id)} className="text-slate-400 hover:text-slate-600 text-lg leading-none">&times;</button>
              </div>
