  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.pageHeader}>
        <h1>Halo {profile?.full_name || "Pengguna"}</h1>
        <p>Pantau progres PKL kamu bulan ini</p>
      </div>

      {!todayEntry && (
        <div className={styles.reminderBanner}>
          <NotebookPen className="h-6 w-6 text-[#95ccdd]" />
          <div>
              ...
              upcomingEvents.map((ev: any) => (
                <div key={ev.id} className="flex items-center gap-3 px-4 py-3">
                  <div
                    className={
                      ev.tipe === "libur"
                        ? styles.eventDotLibur
                        : styles.eventDotEvent
                    }
                  />
              ...
              recentLogbook.map((entry) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-ink truncate max-w-md">
                      {entry.content}
                    </p>
                  </div>
                  {entry.grade !== null ? (
