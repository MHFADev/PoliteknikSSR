...
export default async function AdminOverviewPage() {
  ...
  const [
    ...
    supabase.from("profiles").select("id", { count: "exact", head: true }).eq("role", "pembimbing"),
    supabase.from("attendance_records").select("status, student_id").gte("scanned_at", `${today}T00:00:00`),
    ...
    supabase.from("calendar_events").select("id, title, event_date, tipe").order("event_date", { ascending: true }),
    supabase.from("calendar_events").select("id", { count: "exact", head: true }),
    supabase.from("leave_requests").select("type, start_date, end_date, status, student_id").gte("start_date", sevenDaysAgoStr).lte("start_date", today),
    supabase.from("leave_requests").select("student_id, type, status").lte("start_date", today).gte("end_date", today).in("status", ["disetujui", "pending"]),
    supabase.from("profiles").select("id, full_name, avatar_url, kelas").eq("role", "siswa").order("full_name"),
  ]);
  ...
  const izinToday = todayLeaves?.filter((l) => l.type === "izin" && l.status === "disetujui").length ?? 0;
  const sakitToday = todayLeaves?.filter((l) => l.type === "sakit" && l.status === "disetujui").length ?? 0;
  const cutiToday = todayLeaves?.filter((l) => l.type === "cuti" && l.status === "disetujui").length ?? 0;
  const alfaToday = Math.max(0, (siswaCount ?? 0) - hadirToday - telatToday - izinToday - sakitToday - cutiToday);
  const totalHariIni = hadirToday + telatToday + izinToday + sakitToday + cutiToday + alfaToday;

  // ─── Student Breakdown ─────────────────────────────────────────────
  const studentsWithAttendance = new Set(todayRecords?.map((r) => r.student_id) ?? []);
  const studentsWithLeave = new Set(todayLeaves?.filter((l) => l.status === "disetujui").map((l) => l.student_id) ?? []);
  ...
  const studentBreakdown = (allStudents ?? []).map((s: any) => {
    ...
    let status = "alfa";
    if (hasAttendance) {
      status = todayRecords?.find((r) => r.student_id === s.id)?.status === "hadir" ? "hadir" : "telat";
    } else if (hasLeave) {
      const leave = todayLeaves?.find((l) => l.student_id === s.id && l.status === "disetujui");
      status = leave?.type === "sakit" ? "sakit" : leave?.type === "cuti" ? "cuti" : "izin";
    }
    return { id: s.id, full_name: s.full_name, avatar_url: s.avatar_url, kelas: s.kelas, status };
  });
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    trendMap.set(dateKey, { date: label, hadir: 0, telat: 0, izin: 0, alfa: 0 });
  }
  ...
  return (
    <div className={styles.pageContainer}>
      {/* ─── Header ──────────────────────────────────────────── */}
      <div>
        <h1 className={styles.headerTitle}>Halo, {profile?.full_name || "Admin"}</h1>
        <p className={styles.headerSub}>Gambaran umum seluruh peserta PKL Politeknik SSR</p>
      </div>

      {/* ─── Stat Ringkas — 1 baris, responsive ──────────────── */}
      <div className={styles.statRow}>
        <StatCard label="Siswa" value={siswaCount ?? 0} icon={<GraduationCap className="h-5 w-5" />} accent="biru1" />
        <StatCard label="Pembimbing" value={pembimbingCount ?? 0} icon={<Users className="h-5 w-5" />} accent="teal" />
        <StatCard label="Hadir" value={hadirToday} icon={<CalendarCheck className="h-5 w-5" />} accent="hijau" />
        <StatCard label="Telat" value={telatToday} icon={<Clock className="h-5 w-5" />} accent="steel" />
        <StatCard label="Izin Pending" value={izinPendingCount ?? 0} icon={<FileClock className="h-5 w-5" />} accent="kuning" />
        <StatCard label="Alfa" value={alfaToday} icon={<AlertCircle className="h-5 w-5" />} accent="coral" />
      </div>
            ...
            return (
              <div key={key} className={styles.breakdownItem}>
                <div className={styles.breakdownHeader}>
                  ...
                  <span className={styles.breakdownCount} style={{ color }}>{students.length}</span>
                </div>
                <div className={styles.breakdownStudents}>
                  {students.length > 0 ? (
                    students.map((s) => (
                      <div key={s.id} className={styles.studentRow}>
                        <div className={styles.studentAvatar} style={{ background: s.avatar_url ? "transparent" : bg, color }}>
                          {s.avatar_url ? (
      ...
      <div className={styles.twoCol}>
        <Card className={styles.flipCard}>
          <CardHeader
            ...
            action={<Link href="/dashboard/admin/kalender" className={styles.linkAll}>Lihat Semua</Link>}
          />
          <div className="divide-y divide-outline">
            {upcomingEvents && upcomingEvents.length > 0 ? (
              upcomingEvents.map((ev: any) => (
                <div key={ev.id} className={styles.eventRow}>
                  <div className={ev.tipe === "libur" ? styles.dotLibur : styles.dotEvent} />
